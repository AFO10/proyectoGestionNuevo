import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QWidget, QLineEdit, QPushButton, QTableView, QMessageBox, QLabel, QGroupBox, QFormLayout, QSpinBox, QStyledItemDelegate
...
class VentaWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.sale_model.setHorizontalHeaderLabels(["Producto", "Cantidad", "Precio", "SubTotal", "Acciones"])
    ...
    def add_product_to_sale(self):
        ...
        if selected_indexes:
            for index in selected_indexes:
                ...
                # Añadir una fila a la tabla de venta con botones de aumento/disminución de cantidad
                quantity_item = QStandardItem("1")
                price_item = QStandardItem(f"{product_price:.2f}")
                subtotal_item = QStandardItem(f"{product_price:.2f}")

                row = [QStandardItem(product_name), quantity_item, price_item, subtotal_item]

                # Botones para incrementar y decrementar cantidad
                increment_button = QPushButton("+")
                increment_button.clicked.connect(lambda _, row=row: self.increment_quantity(row))
                decrement_button = QPushButton("-")
                decrement_button.clicked.connect(lambda _, row=row: self.decrement_quantity(row))
    ...
    def update_subtotal(self, row):
        # Actualizar el subtotal basado en la cantidad y el precio
        quantity = int(row[1].text())
        price = float(row[2].text())
        subtotal = quantity * price
        row[3].setText(f"{subtotal:.2f}")
    ...
    def load_styles(self):
        ...
        css_file = QFile("resources/css/vena.css")
...
if __name__ == '__main__':
    ...
    conexion = bd_conectar()  # Crear la conexión a la base de datos
    window = VentaWindow(conexion)
    window.show()
    sys.exit(app.exec_())